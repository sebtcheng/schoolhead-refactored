...
async function checkPendingAssets() {
    try {
        ...
        `);
        console.log("Images (pending):", resImages.rows[0].count);
        ...
        const resDocs = await pool.query(`
            ...
            WHERE ef.approval_status = 'Pending'
        `);
        console.log("Documents (pending):", resDocs.rows[0].count);
        ...
        const resProjDocs = await pool.query(`
            ...
            WHERE ef.approval_status = 'Pending'
        `);
        console.log("ProjectDocs (pending):", resProjDocs.rows[0].count);

        const resFinance = await pool.query(`
            SELECT count(*) 
            FROM co_finance cf
            JOIN engineer_form ef ON cf.project_id = ef.project_id
            WHERE ef.approval_status = 'Pending'
        `);
        console.log("Finance (pending):", resFinance.rows[0].count);

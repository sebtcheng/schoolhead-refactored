...
// GET /api/ph_schools/unit4/:id
router.get('/api/ph_schools/unit4/:id', async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const subRes = await safeQuery(
            'SELECT payload, is_completed, validation_status, validation_remarks FROM ph_school_unit_submissions WHERE iern = $1 AND unit_number = 4',
            [iern]
        );
        ...
        console.error("Unit 4 Fetch Error:", err);
        res.status(500).json({ error: err.message });
    }
});

// PUT /api/ph_schools/unit4/:id
router.put('/api/ph_schools/unit4/:id', async (req, res) => {
    try {
        ...
        const checkLock = await safeQuery(
            'SELECT validation_status, validation_remarks FROM ph_school_unit_submissions WHERE iern = $1 AND unit_number = 4',
            [iern]
        );
        ...
        const upsertRes = await safeQuery(`
            ...
            VALUES ($1, 4, $2::jsonb, $3, $4, CASE WHEN $4 = 'submitted' THEN NULL ELSE $5 END, CASE WHEN $4 = 'submitted' THEN NOW() ELSE NULL END, NOW())
        ...
        console.error("Unit 4 Update Error:", err);
        res.status(500).json({ error: err.message });
    }
});

...
router.put('/api/ph_schools/unit6/:id', async (req, res) => {
  ...
  try {
    ...
    const nextStatus = isResubmission ? 'submitted' : (unit6_completed !== false ? 'submitted' : 'draft');
    ...
    await client.query(`
      ...
      ON CONFLICT (iern, unit_number) DO UPDATE SET
        ...
        submitted_at = NOW(),
        updated_at = NOW()
    `, [iern, JSON.stringify(req.body), nextStatus, nextRemarks]);

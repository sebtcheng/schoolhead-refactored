...
// GET /api/ph_schools/unit2/:id
router.get('/api/ph_schools/unit2/:id', async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const subRes = await safeQuery(
            'SELECT payload, is_completed, validation_status, validation_remarks FROM ph_school_unit_submissions WHERE iern = $1 AND unit_number = 2',
            [iern]
        );
        ...
        console.error("Unit 2 Fetch Error:", err);
        res.status(500).json({ error: err.message });
    }
});

// PUT /api/ph_schools/unit2/:id
router.put('/api/ph_schools/unit2/:id', async (req, res) => {
    try {
        ...
        const upsertRes = await safeQuery(`
            ...
            VALUES ($1, 2, $2::jsonb, $3, $4, CASE WHEN $4 = 'submitted' THEN NULL ELSE $5 END, CASE WHEN $4 = 'submitted' THEN NOW() ELSE NULL END, NOW())

...
router.get('/api/ph_schools/unit9/:id', async (req, res) => {
    try {
        ...
        const subRes = await safeQuery(
            'SELECT payload, is_completed, validation_status, validation_remarks FROM ph_school_unit_submissions WHERE iern = $1 AND unit_number = 9',
            [iern]
        );
        ...
        console.error("Unit 9 Fetch Error:", err);
...
router.put('/api/ph_schools/unit9/:id', async (req, res) => {
    try {
        ...
        const upsertRes = await safeQuery(`
            ...
            VALUES ($1, 9, $2::jsonb, $3, $4, CASE WHEN $4 = 'submitted' THEN NULL ELSE $5 END, CASE WHEN $4 = 'submitted' THEN NOW() ELSE NULL END, NOW())
        ...
        console.error("Unit 9 Update Error:", err);
        res.status(500).json({ error: err.message });
    }
});

...
const Register = ({ isEmbed = false, onBackToLogin }) => {
    ...
    // Load Municipalities directly when Division changes (District level removed)
    useEffect(() => {
        setMunicipalities([]);
    ...
    const handleSchoolSelect = (e) => {
        ...
        const school = availableSchools.find(s => String(s.school_id) === String(schoolId));
        if (school) {
                        ...
                        <div className="mb-8 px-2 animate-in fade-in slide-in-from-top-4 duration-700">
                            ...
                            <div className="flex justify-between mt-2 px-1">
                                <span className="text-[10px] font-black text-[#0470BF] uppercase tracking-widest leading-none">
                                    {currentStep === 1 ? 'Primary Details' : currentStep === 2 ? 'Credentials' : currentStep === 3 ? 'School Selection' : 'Final Security & PIN'}
                                </span>
                                    ...
                                    <div className="space-y-4">
                                        <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200">
                                            ...
                                            <div className="grid gap-3">
                                                <select className="w-full p-3 rounded-xl bg-white border border-slate-200 text-sm outline-none focus:ring-2 focus:ring-blue-500" value={selectedRegion} onChange={(e) => { setSelectedRegion(e.target.value); setSelectedDivision(''); setSelectedMunicipality(''); setSelectedSchool(null); }}>
                                ...
                                >
                                    <div className="space-y-4">
                                        <div className="space-y-1">
                                            ...
                                            <input name="password" type="password" value={formData.password} placeholder="Min. 6 characters" onChange={handleChange} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-blue-500" required />
                                        </div>
                                        <div className="space-y-1">

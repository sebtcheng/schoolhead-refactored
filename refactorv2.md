...
Execute-verbatim implementation prompt for the coding agent. Repo `clea241/InsightEd-SchoolHead-Official-2.0` · branch `schoolhead-refactored` · commit `e43475931cf3ff16606e09783a721f797a4a6dde` (head reconfirmed, no drift). The code at that commit is the source of truth — not this document's prose.

</aside>

## 0. OBJECTIVE & NON-NEGOTIABLES

**Scope (one line):** In `apps/school-head` (+ `packages/shared-io`), fix the four outstanding defects — **R1** unresolved `utils/`/`middleware/` imports (compounded by a broken `@shared/io`), **R2** Unit 6 generic route + mislabeled payload, **R3** unverified Unit 3/4/5/7 endpoints, **R4** dead `autoCleanOldChats()` scheduler — and change nothing else.

**Preserve-list (do NOT modify):**

- DB schema in `packages/shared-db/src/db_init.js`: all tables, columns, conflict keys, `update_unit_timestamp()` trigger, and RLS / Nuclear-Lock policies.
- Join convention: Units 1–5 join on `iern`, Units 6–9 on `school_id`, always filtered by `school_yr`.
- Middleware order + guards in `api/index.js`: `/api/ping` → subpath normalizer → CORS allowlist → `express.json` 500 mb → School-Year Lock → admission control → static `/uploads` → routers.
- School-Year Lock behavior (`403 LOCKED_SCHOOL_YEAR` for `SY 25-26`; default active year `SY 26-27`).
...
3. Quote reference values verbatim; do not paraphrase.
...
5. Single source of truth — the same identifier/value must reconcile everywhere it appears.
6. Conflict precedence: (a) preserve logic → (b) base conventions → (c) explicit revision instruction overrides.
7. Full coverage: each revision maps to exactly one task Rn — nothing dropped, nothing invented.
...
| --- | --- | --- |
| `apps/school-head/api/units/{unit1..unit9,auth,chat,dashboard,docs,location,push,settings}/index.js` | R1 | Migrate relative imports to `@shared/*` |
| `packages/shared-io/src/` (+ `helpers.js`) | R1 | Restore missing `binaryPipeline.js` / `upsertBinary` |
| `apps/school-head/api/units/unit6/index.js`  • `web/src/components/modular/Unit6SchoolResources.jsx` | R2 | Rename route + payload keys |
| `apps/school-head/api/units/{unit3,unit4,unit5,unit7}/index.js` | R3 | Verify/align endpoint paths |
| `apps/school-head/api/index.js` | R4 | Remove or implement `autoCleanOldChats()` |

**Confirmed endpoints (verbatim from synthesis §I.2):**
...
**In-scope tables/columns:** `unit6_school_resources` (+ `unit6_furniture_grades`, `unit6_ecart_batches`); `chat_messages` (`id`, `room_id`, `sender_uid`, `message_text`, `message_type`, `attachment_url`, `attachment_metadata`, `is_read`, `created_at`).
...
**2.1 Offending relative import paths present in the unit routers** (resolving to the absent `apps/school-head/api/utils/` and `apps/school-head/api/middleware/`):
...
**2.2 Confirmed import → `@shared/*` mapping** (verified against each package's `package.json` `exports` field and `src/` — synthesis §II.1):

| Current relative import | Bindings used by routers | Correct specifier |
| --- | --- | --- |
| `../../utils/db.js` | `pool`, `safeQuery`, `cachedQuery`, `poolNew` (named) | `@shared/db` (→ `src/db.js`) |
| `../../utils/updateSchoolTotalCompletion` | `updateSchoolTotalCompletion` (named) | `@shared/db` — same module; exported from `src/db.js`, NOT a separate file |
...
`@shared/db` also exposes the subpath `@shared/db/db_init` (→ `src/db_init.js`). Import forms confirmed in `units/chat/index.js`: `import authMiddleware from '../../middleware/authMiddleware.js'` and `import { pool } from '../../utils/db.js'` — so `authMiddleware` must remain a DEFAULT import after migration to `@shared/auth`. Each `package.json` `exports`: db `{'.':'./src/db.js','./db_init':'./src/db_init.js'}`; auth `{'.':'./src/authMiddleware.js'}`; io `{'.':'./src/helpers.js'}`; all `type: module`.

**2.2a ⚠️ `binaryPipeline.js` / `upsertBinary` is missing everywhere (synthesis §II.2).** `packages/shared-io/src/` holds only `helpers.js` — there is no `binaryPipeline.js`, and `@shared/io` exports only `.` (→ `helpers.js`). Worse, `helpers.js` itself runs `import { upsertBinary } from './binaryPipeline.js'`, so `@shared/io` is itself unresolvable at this commit — independent of the missing app-level `utils/` folder. Restore `upsertBinary` (create `packages/shared-io/src/binaryPipeline.js` and re-export it, e.g. through `helpers.js`) so both `helpers.js` and the `docs` router resolve. **STOP AND ASK:** the canonical `upsertBinary` implementation/source (absent at this commit — cannot be recovered from the repo).

**2.3 Missing folders confirmed uncommitted, NOT gitignored (synthesis §II.3):** root `.gitignore` lists only `node_modules`, `dist`, `.env*`, `uploads/`, `scripts`, `*.sql`/`*.txt`/`*.log`, `data/`, etc. — it does NOT list `utils/`, `middleware/`, or `binaryPipeline`. So the fix is genuine restoration, not un-ignoring.
...
**2.5 Nuclear-Lock deletion bypass** (required only if R4 implements deletion): `SET LOCAL internal.authorized_app_deletion = 'true'` inside the transaction.
...
- **Where:** every router under `apps/school-head/api/units/{unit1..unit9,auth,chat,dashboard,docs,location,push,settings}/index.js`; plus `packages/shared-io/src/`.
- **Do:** replace each relative import per the §2.2 mapping — `../../utils/db.js` → `@shared/db`; `../../utils/updateSchoolTotalCompletion` → `@shared/db`; `../../utils/helpers.js` → `@shared/io`; `../../middleware/authMiddleware.js` → `@shared/auth` (keep the DEFAULT import form). Then repair `@shared/io` per §2.2a: create `packages/shared-io/src/binaryPipeline.js` exporting `upsertBinary` and ensure `helpers.js`'s `import { upsertBinary } from './binaryPipeline.js'` resolves; route `../../utils/binaryPipeline.js` callers (the `docs` router) to `@shared/io`. **STOP AND ASK:** the canonical `upsertBinary` source before writing its body — do not fabricate the PDF-sharding logic.
- **Data bindings:** imports only — no query/column changes.
- **Preserve:** every router's routes, handlers, and behavior; all existing `@shared/*` named exports.

### R2 — Fix Unit 6 route + payload naming

- **Where:** backend `apps/school-head/api/units/unit6/index.js`; frontend `apps/school-head/web/src/components/modular/Unit6SchoolResources.jsx` (confirmed — the only caller of this route).
...
- **Do (frontend — 3 URL sites + payload):** the online submit `fetch(api('/api/ph_schools/<schoolId>'), { method: 'PUT' })` → path `/api/ph_schools/unit6/<schoolId>`; the two offline `addModularToOutbox({ url: api('/ph_schools/<schoolId>'), method: 'PUT' })` sites → `api('/ph_schools/unit6/<schoolId>')`. Rename the same `unit7_*` keys in the `payload` object. Here `schoolId = localStorage.getItem('schoolId')`. NOTE: `api()` strips a leading `api/`, so both URL forms resolve to the same path before and after the rename.
- **Dead keys (optional):** the payload also sends flat `u7_*` fields the unit6 router never reads; leave or delete them, but do not depend on them.
...
- **Preserve:** table targets, conflict key, upsert logic, and the offline/outbox flow; do not touch any other `/api/ph_schools/*` route.

### R3 — Verify Unit 3/4/5/7 endpoints

- **Where:** `apps/school-head/api/units/{unit3,unit4,unit5,unit7}/index.js`.
- **Do:** open each router, document its exact Method + Path, and if any uses a generic collision-prone path like Unit 6's `PUT /api/ph_schools/:id`, align it to `/unitN/:id`. Leave non-colliding endpoints unchanged.
- **Data bindings:** expected conflict keys `(iern, school_yr)` for units 3/4/5 and `(school_id, school_yr)` for unit 7 — verify against `db_init.js`; **STOP AND ASK** if the actual keys differ.
- **Preserve:** upsert logic and `school_yr` default `SY 26-27`.

### R4 — Resolve `autoCleanOldChats()` stub

- **Where:** `apps/school-head/api/index.js`.
...
- [ ]  R3: non-colliding endpoints left untouched; any generic path aligned to `/unitN/:id`.
...
- Units 3/4/5/7 endpoints are documented and collision-free.

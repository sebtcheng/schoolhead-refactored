...
def main():
    ...
    print(f"[3/5] UPLOADING archive to {REMOTE_HOST}...")
    try:
        subprocess.run(f'scp -i "{SSH_KEY_PATH}" {ARCHIVE_NAME} {REMOTE_USER}@{REMOTE_HOST}:~/', shell=True, check=True)
    except subprocess.CalledProcessError:
    ...
    remote_script = (
        "df -h / | grep / | awk '{print $5}' | sed 's/%//' | xargs -I {} sh -c 'if [ {} -gt 90 ]; then echo \"❌ ERROR: Disk usage is at {}%! Deployment aborted for safety.\"; exit 1; fi'; "
        f"mkdir -p {REMOTE_ROOT}/logs && "
        f"tar -xzf ~/{ARCHIVE_NAME} -C {REMOTE_ROOT} && "

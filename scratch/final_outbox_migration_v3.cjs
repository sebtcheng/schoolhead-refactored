...
async function migrate() {
  ...
  try {
    console.log("--- STARTING TWO-TIER ORPHAN MIGRATION (V3) ---");
    ...
    for (const project of orphans) {
      ...
      // Build mapping: Only columns that exist in target AND are not malformed (no spaces)
      const validCols = efColumns.filter(c => {
        // Must match exactly OR be trimmed and match
        return targetCols.has(c);
      });
    ...
    console.log(`\nSUCCESS: Tier 1: ${activeCount}, Tier 2: ${inactiveCount}`);

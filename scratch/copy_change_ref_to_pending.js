...
async function copyData() {
  ...
  try {
    ...
    console.log("Fetching existing school IDs from staging database...");
    const schoolsRes = await pool.query('SELECT school_id FROM ph_schools;');
    const pendingRes = await pool.query('SELECT school_id FROM pending_schools;');
    
    const existingSchoolIds = new Set([
      ...schoolsRes.rows.map(r => r.school_id),
      ...pendingRes.rows.map(r => r.school_id)
    ]);
    console.log(`Staging database has ${schoolsRes.rows.length} schools and ${pendingRes.rows.length} pending schools.`);

    // Fetch an admin user UUID to use as submitted_by
    const userRes = await pool.query("SELECT uid FROM users WHERE role = 'admin' LIMIT 1;");

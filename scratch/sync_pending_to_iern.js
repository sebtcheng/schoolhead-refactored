...
async function syncPendingToIern() {
  try {
    console.log("=== STEP 1: FETCHING CURRENT REFERENCE DATA ===");
    
    // Fetch all existing school ID to IERN mappings
    const refRes = await pool.query('SELECT "SchoolID", "IERN", "status" FROM "schools_IERN";');
    ...
    const maxIdRes = await pool.query('SELECT MAX(id) FROM "schools_IERN";');
    let nextId = (maxIdRes.rows[0].max || 0) + 1;
    console.log(`Next primary key id for schools_IERN: ${nextId}`);

    console.log("\n=== STEP 2: FETCHING PENDING SCHOOLS ===");

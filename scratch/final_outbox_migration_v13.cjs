...
async function migrate() {
  ...
  try {
    console.log("--- STARTING FINAL MIGRATION (V13): IS_DONATED BYPASS ---");
    ...
    console.log(`Unique orphans to process: ${orphans.length}`);
    ...
    for (const project of orphans) {
      ...
      const vals = validCols.map(c => {
         ...
         if (meta.character_maximum_length) {
            ...
            return (strVal.length > meta.character_maximum_length) ? strVal.substring(0, meta.character_maximum_length) : strVal;
         }
         return val;
      });
    ...
    console.log(`\nSUCCESS: Active: ${activeCount}, Inactive: ${inactiveCount}, Skipped: ${skipCount}, Errors: ${errorCount}`);

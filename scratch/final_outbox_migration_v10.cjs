...
async function migrate() {
  ...
  try {
    console.log("--- STARTING MIGRATION V10: DEDUPLICATED BATCH-INSERT ---");
    ...
    // Deduplicate in JS to avoid PK collisions during the loop
    ...
    console.log(`Deduplicated: ${orphans.length} unique orphans from ${orphanRes.rows.length} records.`);
    ...
    // Final Cleanup - Delete projects that now exist in targets
    console.log("Cleanup: Deleting migrated projects from engineer_form...");
    await client.query(`
    ...
    console.log(`\nSUCCESS: Active: ${activeCount}, Inactive: ${inactiveCount}, Skipped (Already Exist): ${skipCount}`);

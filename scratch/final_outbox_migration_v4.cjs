...
async function migrate() {
  ...
  try {
    console.log("--- STARTING TWO-TIER ORPHAN MIGRATION (V4) ---");
    ...
    const efColumns = efColsRes.rows.map(r => r.column_name).filter(c => !c.toLowerCase().includes('total columns'));
    ...
    for (const project of orphans) {
      ...
      // engineer_create has PK on project_id. 
      // engineer_form_outbox might not, but we should be safe.
      try {
        await client.query(`
          INSERT INTO ${targetTable} (${colStr}) 
          VALUES (${valStr})
          ON CONFLICT (project_id) DO UPDATE SET 
            ipc = EXCLUDED.ipc,
            engineer_id = EXCLUDED.engineer_id
          WHERE ${targetTable}.project_id = EXCLUDED.project_id
        `, vals);
        
        if (isActive) activeCount++; else inactiveCount++;
      } catch (err) {
        if (err.code === '42703' || err.code === '42P01') { 
          // Column and table errors should fail the whole script
          throw err; 
        }
        // PK errors in tables WITHOUT project_id primary key will fall through 
        // to a general INSERT if ON CONFLICT fails.
        // Let's use a simpler approach: Just check existence.

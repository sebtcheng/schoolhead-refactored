...
const PHOTO_DESCRIPTIONS = {
  Internal:
    "Required: Snapshots of interior progress, structural elements, or indoor finishing.",
  External:
    "Required: Snapshots of facade, site perimeter, or overall building exterior.",
};
...
const CONSTRUCTION_OPTIONS = [
  ...
  {
    value: "For Final Inspection",
    label: "For Final Inspection",
    icon: "🔍",
    color: "purple",
  },
...
const BIDDING_MILESTONES = [
  ...
  { key: "requestForQuotation", label: "Request for Quotation" },
  { key: "negotiation", label: "Negotiation" },
  { key: "openingOfQuotation", label: "Opening of Quotation" },
...
const PhotoCard = ({
  ...
  return (
    <div
      className={`p-5 rounded-3xl border-2 border-dashed transition-all duration-300 ${categoryColor.bg} ${categoryColor.border}`}
    >
      <div className="flex flex-col items-center text-center space-y-4">
        <div
          className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-sm ${categoryColor.iconBg} ${categoryColor.icon}`}
        >
          <FiCamera size={26} />
        </div>
        <div>
          <h4
            className={`text-sm font-black uppercase tracking-tight ${categoryColor.text}`}
          >
            {activePhotoCategory} Photos
          </h4>
            ...
            <div className="grid grid-cols-4 gap-2">
              {previews.map((src, i) => (
                <div key={i} className="relative aspect-square group">
                  <img
                    src={src}
                    className="w-full h-full object-cover rounded-xl shadow-sm ring-1 ring-black/5"
                  />
                  <button
                    onClick={() => removePhoto(activePhotoCategory, i)}
...
const UpdateProjectWizard = ({ project, onSave, onClose, isOpen }) => {
  ...
  const [constructionStatus, setConstructionStatus] = useState(
    project?.status || ConstructionStatus.NotYetStarted,
  );
  const [percentage, setPercentage] = useState(project?.percentage || 0);
  const [actualCompletionDate, setActualCompletionDate] = useState(
    project?.actualCompletionDate || "",
  );
  const [statusAsOfDate, setStatusAsOfDate] = useState(
  ...
  const [biddingDates, setBiddingDates] = useState({
    ...
    issuanceOfInvitationToBid:
      project?.issuanceOfInvitationToBid || project?.adPostDate || "",
    preBidConference:
      project?.preBidConference || project?.preBidConfDate || "",
    ...
    requestForQuotation:
      project?.requestForQuotation || project?.request_for_quotation || "",
    negotiation: project?.negotiation || "",
    openingOfQuotation:
      project?.openingOfQuotation || project?.opening_of_quotation || "",
  ...
  const triangulatedPercentage = Object.entries(checkedState).reduce(
    (acc, [id, checked]) => {
  ...
  const getNextStep = () => {
    ...
    if (step === 1)
      return isProcurementComplete || wasAlreadyInConstruction ? 2 : 5;
    if (step === 2)
      return isConstructionActive || wasAlreadyInConstruction ? 3 : 5;
    return step + 1;
  };
  ...
  const removePhoto = (category, index) => {
    if (category === "Internal") {
      ...
      setActivePreviews((prev) => ({
        ...prev,
        Internal: prev.Internal.filter((_, i) => i !== index),
      }));
    } else {
  ...
  const canProceedNext = () => {
    if (step === 1) {
      if (!procurementStatus)
        return { ok: false, reason: "Please select a procurement status." };
      if (REASON_OPTIONS[procurementStatus] && selectedReasons.length === 0) {
        return {
          ok: false,
          reason: "Please select at least one justification/reason.",
        };
      }
      if (isProcurementComplete) {
        if (!biddingDates.dateNoticeOfAward)
          return {
            ok: false,
            reason: "Notice of Award Date is required for 'Complete' status.",
          };
        if (!contractAward.contractId)
          return {
            ok: false,
            reason: "Contract ID / Number is required for 'Complete' status.",
          };
        // Mandatory timeline fields for BEFF (non-donated) projects
        const isBeff = !project?.is_donated && !project?.isDonated;
        if (isBeff) {
          if (!contractAward.noticeToProceed)
            return {
              ok: false,
              reason:
                "Notice to Proceed (NTP) date is required for BEFF projects.",
            };
          if (!contractAward.targetCompletionDate)
            return {
              ok: false,
              reason: "Target Completion Date is required for BEFF projects.",
            };
        }
      }
      return { ok: true };
    ...
    if (step === 2) {
      ...
      if (
        ...
        return {
          ok: false,
          reason: "Please enter the Actual Completion Date.",
        };
      }
      return { ok: true };
    }
    ...
    if (step === 3) {
      ...
      if (
        ...
        return {
          ok: false,
          reason: `Please upload both Internal and External photos for "${constructionStatus}" status. Missing: ${missing.join(" & ")}.`,
        };
      }
    }
    return { ok: true };
  ...
  const handleSubmit = () => {
    setIsUploading(true);
    let mappedReasons = selectedReasons.map(r => r === "Others" && customReason.trim() ? `Others (${customReason.trim()})` : r);
    const finalRemarks =
      mappedReasons.length > 0
        ? `[Justification: ${mappedReasons.join(", ")}] ${remarks}`.trim()
        : remarks;

    const finalStatus = isProcurementComplete
      ? constructionStatus
      : project.status;
    const finalPercentage = isProcurementComplete
      ? percentage
      : project.accomplishmentPercentage ||
        project.accomplishment_percentage ||
        0;
    ...
    onSave(
      {
        ...
        actualCompletionDate: isProcurementComplete
          ? actualCompletionDate
          : project.actualCompletionDate || "",
        statusAsOfDate,
        remarks: finalRemarks,
        statusDesignPhase: isProcurementComplete
          ? "Completed"
          : project?.statusDesignPhase || "Ongoing",
        triangulatedPercentage: isProcurementComplete
          ? triangulatedPercentage
          : project.triangulatedPercentage || 0,
        ...biddingDates,
        ...contractAward,
        ...details,
  ...
  const getStatusColor = (status) => {
    ...
    if (
      ...
      status === ProcurementStatus.ProcurementComplete
    )
      return {
        bg: "bg-emerald-50",
    ...
    if (
      status === ConstructionStatus.Suspended ||
      status === ConstructionStatus.Terminated
    )
      return {
        bg: "bg-red-50",
        text: "text-red-600",
  ...
  const displayedConstructionStatus = isProcurementComplete
    ? constructionStatus
    : project?.status || ConstructionStatus.NotYetStarted;
  const constCols = getStatusColor(displayedConstructionStatus);

  const categoryColor =
    activePhotoCategory === "Internal"
      ? {
          bg: "bg-blue-50",
      ...
      <div className="bg-white w-full max-w-sm h-full max-h-[700px] rounded-[30px] shadow-2xl flex flex-col overflow-hidden relative border border-white/20">
        ...
        <div className="px-6 pt-6 pb-4 bg-white/80 backdrop-blur-xl border-b border-slate-50 relative shrink-0">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              onClick={handleClose}
              className="w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 hover:bg-slate-100 active:scale-95 transition-all"
            >
              <FiX size={16} />
            </button>
          </div>
            ...
            {visibleSteps.map((s, idx) => (
              <React.Fragment key={s.id}>
                <div className="flex flex-col items-center gap-1">
                  <div
                    className={`h-1 tracking-tighter rounded-full transition-all duration-500 ${step >= s.id ? "bg-blue-600 w-12" : "bg-slate-100 w-4"}`}
                  />
                  <span
                    className={`text-[8px] font-black uppercase tracking-[0.1em] ${step === s.id ? "text-blue-600" : "text-slate-300"}`}
                  >
                    {s.label}
                  </span>
                </div>
                {idx < visibleSteps.length - 1 && (
                  <div className="flex-1 h-[1px] bg-slate-50 mb-2.5 mx-1" />
                )}
              </React.Fragment>
            ))}
          </div>
        ...
        <div ref={bodyRef} className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4">
                    ...
                    {REASON_OPTIONS[procurementStatus].map((reason, idx) => {
                      ...
                      return (
                        <div key={idx} className="flex flex-col gap-2">
                          <label
                            className={`flex items-start gap-3 p-3 rounded-xl border transition-all cursor-pointer ${isSelected ? "bg-blue-50 border-blue-200" : "bg-white border-slate-100 hover:border-slate-200"}`}
                          >
                            <div
                              className={`mt-0.5 w-4 h-4 rounded flex-shrink-0 border-2 transition-all flex items-center justify-center ${isSelected ? "bg-blue-600 border-blue-600" : "bg-white border-slate-300"}`}
                            >
                              {isSelected && (
                            ...
                            <span
                              className={`text-[10px] font-bold leading-relaxed ${isSelected ? "text-blue-700" : "text-slate-600"}`}
                            >
                              {reason}
                            </span>
                          </label>
                          {isSelected && reason === "Others" && (
                            <input
                              type="text"
                              placeholder="Please specify your reason here..."
                              value={customReason}
                              onChange={(e) => setCustomReason(e.target.value)}
                              className="p-3 rounded-xl border border-blue-200 bg-white text-xs font-bold text-slate-700 outline-none ml-6"
                            />
                          )}
                        </div>
                      );
              ...
              {[
                ...
                <div className="animate-in fade-in slide-in-from-top-2 duration-300 bg-blue-50 p-3 sm:p-4 rounded-2xl border border-blue-100">
                  ...
                  <div className="space-y-6">
                    {/* Always show Bidding Milestones when Under Procurement or Complete */}
                    ...
                    {[ProcurementStatus.UnderProcurement, ProcurementStatus.ProcurementComplete].includes(procurementStatus) && (
                      <div className="mt-8 pt-6 border-t border-blue-200 space-y-4">
                        ...
                        <div>
                          <label className="block text-[9px] font-black text-blue-400 uppercase tracking-widest mb-1.5">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setBiddingDates((prev) => ({
                                ...prev,
                                dateNoticeOfAward: e.target.value,
                              }))
                            }
                        ...
                        {/* Project Timeline — BEFF projects only */}
                        {!project?.is_donated && !project?.isDonated && (
                          <div className="mt-2 pt-4 border-t border-blue-100 space-y-3">
                            ...
                            <div>
                              ...
                              <input
                                type="date"
                                value={contractAward.noticeToProceed}
                                onChange={(e) =>
                                  setContractAward((prev) => ({
                                    ...prev,
                                    noticeToProceed: e.target.value,
                                  }))
                                }
                                className="w-full p-2.5 rounded-xl border border-blue-200 bg-white text-xs font-bold text-slate-700 outline-none"
                              />
                            ...
                            <div>
                              ...
                              <input
                                type="date"
                                value={contractAward.constructionStartDate}
                                onChange={(e) =>
                                  setContractAward((prev) => ({
                                    ...prev,
                            ...
                            <div>
                              ...
                              <input
                                type="date"
                                value={contractAward.targetCompletionDate}
                                onChange={(e) =>
                                  setContractAward((prev) => ({
                                    ...prev,
          ...
          {step === 2 && (
            <div className="space-y-5 animate-in fade-in slide-in-from-bottom-2 duration-300">
              ...
              <div>
                ...
                >
                  ...
                  {CONSTRUCTION_OPTIONS.map((opt) => (
                    <option key={opt.value} value={opt.value} disabled={false}>
                      {opt.icon} {opt.label}
                    </option>
                  ))}
                </select>
                  ...
                  <div className="space-y-2 max-h-[200px] overflow-y-auto pr-2 custom-scrollbar">
                    {REASON_OPTIONS[constructionStatus].map((reason, idx) => {
                      ...
                      return (
                        <div key={idx} className="flex flex-col gap-2">
                          <label
                            className={`flex items-start gap-3 p-3 rounded-xl border transition-all cursor-pointer ${isSelected ? "bg-amber-50 border-amber-200" : "bg-white border-slate-100 hover:border-slate-200"}`}
                          >
                            <div
                              className={`mt-0.5 w-4 h-4 rounded flex-shrink-0 border-2 transition-all flex items-center justify-center ${isSelected ? "bg-amber-600 border-amber-600" : "bg-white border-slate-300"}`}
                            >
                              {isSelected && (
                                <FiCheck size={10} className="text-white" />
                              )}
                            ...
                            <span
                              className={`text-[10px] font-bold leading-relaxed ${isSelected ? "text-amber-700" : "text-slate-600"}`}
                            >
                              {reason}
                            </span>
                          </label>
                          {isSelected && reason === "Others" && (
                            <input
                              ...
                              value={customReason}
                              onChange={(e) => setCustomReason(e.target.value)}
                              className="p-3 rounded-xl border border-amber-200 bg-white text-xs font-bold text-slate-700 outline-none ml-6"
                            />
                          )}
                        </div>
                      );
              ...
              {constructionStatus === ConstructionStatus.Completed && (
                <div>
                  <label className="block text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-2">
                    Actual Completion Date *
                  </label>
                  <input
                    type="date"
          ...
          {step === 3 && (
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
              ...
              <div
                className={`p-4 rounded-2xl border ${constCols.bg} ${constCols.border}`}
              >
                <div className="flex items-center justify-between mb-3">
                  <label
                    className={`text-[10px] font-black uppercase tracking-widest ${constCols.text}`}
                  >
                    Accomplishment %
                  </label>
                  <span
                    className={`text-xl font-black tabular-nums ${constCols.text}`}
                  >
                    {percentage}%
                  </span>
                </div>
              ...
              <p className="text-[11px] text-slate-500 font-medium leading-relaxed px-1">
                Attach internal and external site photos. Required for{" "}
                <span className="font-black text-blue-600">
                  Ongoing, For Final Inspection, and Completed
                </span>{" "}
                statuses.
              </p>
              ...
              <input
                ...
                multiple
                className="hidden"
                onChange={(e) => handlePhotoSelect(e, "Internal")}
              />
              <input
              ...
              <input
                ...
                multiple
                className="hidden"
                onChange={(e) => handlePhotoSelect(e, "External")}
              />
              <input
                ref={externalCameraRef}
                type="file"
                accept="image/*"
                capture="environment"
                className="hidden"
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
              <div className="bg-blue-50 p-4 rounded-2xl border border-blue-100">
                ...
                <p className="text-[9px] text-blue-400 font-bold mb-3">
                  {project?.numberOfStoreys || 1}-Storey building ·{" "}
                ...
                <div className="flex items-center justify-between text-[9px] font-bold">
                  ...
                  {Math.abs(triangulatedPercentage - percentage) > 10 && (
                    <span className="text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full border border-amber-200">
                      ⚠ {Math.abs(triangulatedPercentage - percentage)}%
                      variance
                    </span>
                  )}
                </div>
              ...
              <div className="space-y-2">
                {checklist.map((task) => (
                  ...
                  >
                    <div
                      className={`w-5 h-5 rounded-lg border-2 flex items-center justify-center shrink-0 transition-all ${checkedState[task.id] ? "bg-emerald-500 border-emerald-500" : "border-slate-200"}`}
                    >
                    ...
                    <span
                      className={`text-[11px] font-bold flex-1 leading-tight ${checkedState[task.id] ? "text-emerald-700" : "text-slate-600"}`}
                    >
                      {task.task}
                    </span>
                    <span
                      className={`text-[9px] font-black px-1.5 py-0.5 rounded-full shrink-0 ${checkedState[task.id] ? "bg-emerald-100 text-emerald-600" : "bg-slate-50 text-slate-400"}`}
                    >
                      {task.weight}%
                    </span>
                  </button>
          ...
          {step === 5 && (
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
              ...
              <div className="space-y-3">
                <div
                  className={`p-3 rounded-2xl border ${procChanged ? `${procCols.bg} ${procCols.border}` : "bg-slate-50 border-slate-100"}`}
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-[11px] font-black ${procCols.text}`}>
                      {procurementStatus}
                    </span>
                    {procChanged && (
                      <span
                        className={`ml-auto text-[8px] font-black px-2 py-0.5 rounded-full border bg-white border-current uppercase ${procCols.text}`}
                      >
                        Changed
                      </span>
                    )}
                ...
                {constructionStatus && (
                  <div
                    className={`p-3 rounded-2xl border ${constChanged ? `${constCols.bg} ${constCols.border}` : "bg-slate-50 border-slate-100"}`}
                  >
                    <p className="text-[8px] font-black uppercase tracking-widest text-slate-400 mb-1.5">
                      Construction Status
                    </p>
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] font-bold text-slate-500 line-through">
                        {originalConstruction || "N/A"}
                      </span>
                      <span className="text-slate-300">→</span>
                      <span
                        className={`text-[11px] font-black ${constCols.text}`}
                      >
                        {displayedConstructionStatus}
                      </span>
                      {(constChanged || !isProcurementComplete) && (
                        <span
                          className={`ml-auto text-[8px] font-black px-2 py-0.5 rounded-full border bg-white border-current uppercase ${constCols.text}`}
                        >
                ...
                {isConstructionActive && (
                  <div className="p-3 rounded-2xl border bg-slate-50 border-slate-100">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span
                        className={`text-[11px] font-black ${Math.abs(triangulatedPercentage - percentage) > 10 ? "text-amber-600" : "text-emerald-600"}`}
                      >
                ...
                <div
                  className={`p-3 rounded-2xl border ${internalFiles.length + externalFiles.length > 0 ? "bg-blue-50 border-blue-200" : "bg-slate-50 border-slate-100"}`}
                >
                  <p className="text-[8px] font-black uppercase tracking-widest text-slate-400 mb-1.5">
                    Site Photos
                  </p>
                  <span className="text-[11px] font-black text-blue-600">
                    {internalFiles.length > 0 &&
                      `${internalFiles.length} Internal`}
                    {internalFiles.length > 0 &&
                      externalFiles.length > 0 &&
                      ", "}
                    {externalFiles.length > 0 &&
                      `${externalFiles.length} External`}
                    {internalFiles.length === 0 &&
                      externalFiles.length === 0 && (
